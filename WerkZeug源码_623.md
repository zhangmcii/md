...
解释：
1.这里的__slots__中定义的__local在哪里赋值的，没有赋值，_get_current_object()函数一开始检测__local，还能返回self.__local()？
真正赋值在 __init__里，
```
object.__setattr__(self, '_LocalProxy__local', local)
```
这行其实就是在给 self.__local 赋值。因为类里面的 __xxx 会被改写为：_ClassName__xxx，
所以：__local -> _LocalProxy__local

只是作者为了：避免触发 __setattr__, 直接调用底层赋值,所以用了 object.__setattr__。
