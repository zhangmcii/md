...
- 创建和销毁成本极低。一个线程需要 ~1MB 栈空间，一个协程只需要几KB，相当于少了1000倍
...
<details>
<summary>为什么 async 能轻松支撑 10 万连接，而线程不行？</summary>
async 能撑 10 万连接，本质原因不是“更快”, 而是每个连接几乎不占资源

我们可以从“一个连接到底占什么？”， “连接大多数时间在干嘛”两个角度分析
1.一个连接到底占什么？
线程开销：
栈内存：≈ 1MB（有的更大）
内核调度结构
...
10万连接：100000 × 1MB = 100GB 内存 ❌（直接炸）
协程开销：
几 KB（甚至更少）
Python对象 + 状态机

10万连接：100000 × 几KB ≈ 几百MB ✅（可以接受）
...
<details>
<summary>相同规范的并发，async比线程池的用时更短。你说async不是因为它“更快”，如何理解？</summary>
原因不是“单个任务执行更快”，而是系统整体浪费更少
线程切换成本：
内核态切换
保存/恢复寄存器
CPU cache 失效
...
## 什么是 Python ⾯向对象的封装特性？
封装的本质是把对象的属性和操作这些属性的⽅法绑在⼀起，对外隐藏内部实现细节。你调⽤⼀个⽅法时，压根不关⼼它内部怎么实现的，只要知道这个接⼝能做什么就⾏。
...
本质上返回一个新的具有命名字段的元组子类。能通过字段名访问元素，⽐普通元组可读性强得多。
...
def fetch_url(url):
    ...
    print(f"{url}: {len(resp.content)}")


t1 = threading.Thread(target=fetch_url, args=("https://httpbin.org/delay/1",))
t2 = threading.Thread(target=fetch_url, args=("https://httpbin.org/delay/1",))
t1.start()
t2.start()
t1.join()
t2.join()
...
## 什么是 Python 的 序列化(pickling) 和 反序列化(unpickling) ？
Python 的 pickling 是把 Python 对象转成字节流的过程，⽅便存⽂件或⽹络传输。反过来，unpickling 就是把字
节流重新还原成原来的 Python 对象。

这个机制底层靠的是 pickle 模块实现，它能处理⼤多数内置类型和⾃定义类实例。⽐如你有个字典或者⼀个训练好的机器学习模型（像 scikit-learn 的模型），想保存下来以后⽤，直接 pickle 到⽂件就⾏。
1.序列化⽰例： pickle.dump(data, file)
```
import pickle
...
## python的闭包
函数能记住它被定义时所处的环境，哪怕这个环境已经消失了
1） 当⼀个内部函数引⽤了外层函数的变量，这个内部函数就变成了闭包。外层函数返回后，内部函数依然可以访问那些变量，不会因为外层函数调⽤结束就被回收。
2） 闭包的关键在于“延迟绑定”和“变量捕获”。Python 的作⽤域规则决定了内部函数会保留对外部⾃由变量的引⽤。
3） 常⻅⽤途⽐如实现装饰器、回调函数、状态保持。
```
...
```
上⾯这段代码⾥， multiplier 函数记住了 n=2 ，即使 make_multiplier 已经执⾏完了。这就是闭包在起作⽤。

闭包让函数带上了上下⽂，相当于⼀个轻量级的对象实例。
注意，循环中创建闭包可能引发的陷阱，⽐如 lambda在列表推导式中容易绑定到同⼀个变量引⽤。

## 列表的append，insert, extend
1.append 是往列表末尾加⼀个元素，不管这个元素是啥类型，它都当做⼀个整体塞进去。
2.insert 是在指定位置插⼊⼀个元素，后⾯的元素⾃动往后挪。⽐如 lst.insert(0, 'x') 就是在开头插⼊，原来的元素都顺延
3.extend 才是真正⽤来“扩展”列表的，它会把⼀个可迭代对象⾥的每个元素逐个拿出来，追加到原列表后⾯。⽐如lst.extend([1, 2, 3]) 相当于连续三次 append ，但效率更⾼，代码也更⼲净。

## 操作列表元素时， remove 、 del 和 pop 
1.remove 是按值删除，你告诉它“删掉哪个具体的值”，它就去找第⼀个匹配的删了。⽐如 list.remove(5) 会删
掉第⼀个等于 5 的元素。要是这值压根不存在，直接抛 ValueError 。它不返回任何东西，就是默默删掉。

2.del 是个通⽤语句，能删列表⾥的单个元素、切⽚，甚⾄整个变量。它不关⼼值，只看位置。 del list[0] 删第
⼀个元素， del list[1:3] 删⼀段， del list 连变量都删掉。它也不返回被删内容。

3.pop 是按索引删，默认删最后⼀个，还能把删掉的那个值返回回来。这就很适合⽤在“边取边删”的场景，⽐如当
栈⽤的时候。 pop(0) 虽然能删第⼀个，但性能差，因为要整体前移。



## python字典
Python 的字典（dict）本质是⼀个哈希表，⽤键值对存储数据，查找平均时间复杂度是 O(1)。

使用dict() 构造函数，用键值对序列创建字典或者使用花括号 {} 创建字典

注意：
1.字典的 key 必须是不可变类型
2.避免在⾼并发写场景直接⽤ dict，要加锁。


## 可迭代对象(iterables)和迭代器(iterators)的区别
关键区别在于：可迭代对象每次 for 都能从头开始，因为每次都调 __iter__() 拿新迭代器；⽽迭代器⼀旦⾛到底， __next__() 就⼀直抛异常，相当于“耗尽”了。

(1)iterables 是能被 for 循环遍历的对象，⽐如 list、tuple、str、dict、set。它们的共同点是实现了__iter__() ⽅法，调⽤后会返回⼀个迭代器。平时写 for x in [1,2,3] ，背后的机制就是先拿这个列表的迭代器。


（2）iterators 才是真正实现“⼀次取⼀个”逻辑的对象。它必须有 __iter__() 和 __next__() ⽅法。__next__() 每次返回下⼀个值，到头了就抛 StopIteration 。重点是，迭代器本⾝也是可迭代的，但它迭代⾃⼰时， __iter__() 直接返回⾃⼰，不会新建状态。


## map函数

map() ⽤来对可迭代对象中的每个元素**批量执⾏**同⼀个函数操作。它不会修改原数据，⽽是返回⼀个新的迭代器，⾥⾯是处理后的结果。

第⼀个参数是函数名，后⾯跟⼀个或多个可迭代对象。

⽐如你想把列表⾥每个数字转成字符串⻓度：
```
list(map(len, ['hello', 'world', 'python'])) # 输出 [5, 5, 6]
```

如果要对多个序列同时操作，⽐如把两个列表对应位置的数相加：
```
list(map(lambda x, y: x + y, [1, 2, 3], [4, 5, 6])) # 输出 [5, 7, 9]
```
这里注意， map() 返回的是 map 对象，不是列表。你要么⽤ list() 转⼀下，要么在循环⾥直接⽤。


## Python 的 **init** 和 **new** ⽅法有什么区别？
__new__ 是真正创建实例的⽅法，⽽ __init__ 只是负责初始化已创建的实例
1） __new__ 是⼀个类⽅法，它在实例创建之前被调⽤，接收的第⼀个参数是类本⾝，并返回⼀个该类的实例。如果你想要控制对象的创建过程，⽐如实现单例模式或不可变类型，就得重写它。
单例模式：
```
class Singleton:
    _instance = None
...
自定义不可变字符串：
```
<!-- 不可变对象在“创建时”就必须确定值，不能在 __init__ 里再改, 所以必须在__new__中完成初始化 -->
class MyStr(str):
    def __new__(cls, value):
        value = value.upper()  # 在创建时修改
...
2） __init__ 是在 __new__ 返回实例之后⾃动调⽤的，它的作⽤是给这个已经存在的实例“填数据”。你常⻅的
属性赋值 self.name = name 都放在这⾥。它没有返回值，只是“装修”对象。


典型场景是 需要缓存实例时操作 __new__ ，⽐如 type.__new__ 在构建类的时候起作⽤。⼤多数情况下我们只⽤ __init__ ，因为不需要⼲预对象创建过程。


## 什么是猴子补丁（monkey patching）？
“猴⼦补丁”指的是在程序运⾏时动态修改类或模块的⾏为，不⽤改源码也能让⽼代码执⾏新逻辑。这招在 Python ⾥特别常⻅，Java 这种静态语⾔基本没这说法。

⼀般⽤在框架层⾯做透明增强，⽐如你想给某个第三⽅库的函数加个⽇志埋点，但⼜不想 fork 项⽬⾃⼰维护，就可以
在应⽤启动时“打补丁”，把原⽅法替掉或者包装⼀层。

多个地⽅都去改同⼀个⽅法，冲突了很难查；调试时发现实际执⾏的不是源码⾥的逻辑，直接懵圈。所以⼀般来说，业务代码别乱⽤，框架级⼯具可以谨慎考虑。